...
export interface PickedICloudFile {
  uri: string;
...
type Category = 'Images' | 'Videos' | 'Audio' | 'Documents' | 'Other';
...
const CATEGORY_ORDER: Category[] = ['Images', 'Videos', 'Audio', 'Documents', 'Other'];
...
export default function ICloudBackupModal({
  visible, files, quotaBytes, usedBytes, onClose, onDone, onStoragePurchased,
}: Props) {
  ...
  // Reset when modal opens with new files
  useEffect(() => {
    if (!visible || files.length === 0) return;
    const mapped = files.map((f) => ({
      uri: f.uri,
      copyUri: f.fileCopyUri,
  ...
  const fitsPct = quotaBytes > 0
    ...
    : 0;
  // Red bar fills the remaining space when over quota
  ...
  const typeSections: TypeSection[] = useMemo(() => {
    ...
    entries.forEach((entry, index) => map.get(getCategory(entry.type))!.push({ entry, index }));
  ...
  const handleBackUp = async () => {
    ...
    for (let i = 0; i < toUpload.length; i++) {
      const e = toUpload[i];
      try {
        await uploadFile(e.copyUri ?? e.uri, e.name, e.type, e.destFolderId ?? undefined);
      } catch {
        errors++;
      }
  ...
  const handleCleanup = () => {
    ...
    Alert.alert(
      ...
      `Remove ${count} backed-up file${count !== 1 ? 's' : ''} from iCloud Drive? They are safely stored in Apollo SFS.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: `Delete ${count} file${count !== 1 ? 's' : ''}`,
          style: 'destructive',
          onPress: async () => {
            setCleanupLoading(true);
            let failed = 0;
            for (const e of entries) {
              const path = (e.copyUri ?? e.uri).replace(/^file:\/\//, '');
              try { await RNBlobUtil.fs.unlink(path); }
              catch { failed++; }
            }
            setCleanupLoading(false);
            Alert.alert(
              failed === 0 ? 'Done' : 'Partial cleanup',
              failed === 0
                ? 'All backed-up files have been removed from iCloud Drive.'
                : `${count - failed} removed. ${failed} could not be deleted.`,
            );
          },
        },
      ],
  ...
  const renderRow = ({ entry, index }: ListItem) => {
    ...
    const Icon = fileIcon(entry.type);
    return (
      <TouchableOpacity
      ...
      >
        ...
        <View style={styles.fileIconWrap}>
          <Icon size={18} color={checked ? colors.primary : colors.textMuted} strokeWidth={1.5} />
        </View>
    ...
    >
      <View style={styles.root}>
        ...
        <View style={styles.footer}>
          ...
          {finished && (
            <TouchableOpacity
              style={[styles.cleanupBtn, cleanupLoading && styles.btnDisabled]}
              ...
              {cleanupLoading ? (
                ...
                <>
                  ...
                  <Text style={styles.cleanupBtnText}>
                    Delete {entries.length} backed-up file{entries.length !== 1 ? 's' : ''} from iCloud
                  </Text>
                </>
              )}
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  progressFill: { height: '100%', backgroundColor: colors.info, borderRadius: radius.xl },
