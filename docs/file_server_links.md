...
- **Upload and download only.** `GET`/`HEAD`/`PROPFIND` (download/browse) and
  `PUT`/`MKCOL` (upload) are the only mutating-capable verbs allowed.
  `DELETE`, `MOVE`, `COPY` and `PROPPATCH` return 403, and `PUT` refuses to
  overwrite an existing file (overwrite would be deletion by another name).
- **No previews, no execution.** Every download is served as
  ...
  executed or rendered server-side.
- **Server-scoped.** The mount exposes only files stored on the link's
  server's drives, and uploads are hard-pinned to the owner's allocated drive
  on that server (no fallback routing to other servers).
...
When enabled, uploads/downloads are only allowed from **verified locations**
(source IPs). A transfer from a new IP — or one whose verification is older
than 30 days — is rejected with 403 and a verification email is sent to the
owner (throttled to one per 10 minutes per location). The email links to
...
| DAV handler | `api/routes/dav/handler.go` (registered at `/dav/:token`, outside `/api/v1`) |
