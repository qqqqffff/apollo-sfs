...
// The leading "home" is always the root sentinel. When the path doesn't fit
...
export function FolderBreadcrumb({ folderId, onNavigate, trailing }: Props) {
  ...
  const totalChars = segments.reduce((n, f) => n + f.name.length + 3, 0) + 19 // 19 = leading "home"
  ...
  if (!fits && segments.length > 1) {
    ...
    let chars = 19 + (leaf ? leaf.name.length : 0) + 6 // "home" + leaf + ".." section
