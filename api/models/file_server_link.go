...
// FileServerLink is a premium WebDAV mount link scoped to one storage server.
...
// HTTP Basic auth, which are verified against Keycloak.
type FileServerLink struct {
	...
	DriveID  uuid.UUID `json:"-"`
	// ServerName is joined from servers for display; not stored on the row.
